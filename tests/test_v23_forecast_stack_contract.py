...
class ForecastAdapterContractTests(unittest.TestCase):
    ...
    def test_moirai_uni2ts_uses_standard_statuses(self) -> None:
        service = ForecastProviderStatusService(
            {"forecast_stack": {"providers": {"moirai_uni2ts": {
                "enabled": True, "priority": 3, "role": "experimental_foundation_benchmark",
...
class ForecastAccuracyAndSnapshotContractTests(unittest.TestCase):
    ...
    def _forecast(self, *, generated_at: str | None = None) -> dict:
        return {
            ...
            "forecast_expected_return_pct": 5.0, "confidence": "HIGH",
            "entry_trigger_reference": 103.0, "stop_level_reference": 97.0,
        }

    def _setup(self, setup_id: str, snapshot: MarketSnapshot | None = None) -> SetupCreationSnapshotService:
        config = {
            "setup_id": setup_id, "symbol": "SNAP", "setup_type": "momentum_breakout",
    ...
    def test_reliability_grade_insufficient_data(self) -> None:
        self.assertEqual(
            reliability_grade({"sample_size": 1, "direction_accuracy": 1, "mape": 0}, min_samples=2),
            "INSUFFICIENT_DATA",
        )

    def test_reliability_grade_A_B_C_D_F(self) -> None:
        cases = [(0.7, 0.03, "A"), (0.6, 0.05, "B"), (0.54, 0.07, "C"), (0.49, 0.11, "D"), (0.2, 0.5, "F")]
        self.assertEqual(
            [reliability_grade({"sample_size": 30, "direction_accuracy": accuracy, "mape": mape}) for accuracy, mape, _ in cases],
            [grade for _, _, grade in cases],
        )

    def test_setup_creation_snapshot_created_on_new_setup(self) -> None:
        service = self._setup("SNAP_A", MarketSnapshot(symbol="SNAP", price=10.5, bid=10.4, ask=10.6))
    ...
    def test_snapshot_does_not_replace_entry_price(self) -> None:
        ...
        self.assertEqual(self.repository.get_setup("SNAP_C")["config"]["entry"]["trigger_price"], 11.0)
    ...
    def test_duplicate_setup_creates_new_snapshot(self) -> None:
        ...
        second = self._setup("SNAP_E2", MarketSnapshot(symbol="SNAP", price=10.5)).capture("SNAP_E2")
...
class ForecastEnsembleAndModelLabContractTests(unittest.TestCase):
    ...
    def _forecast(model: str, direction: str, **extra):
        return {"model": model, "status": "OK", "direction": direction, "forecast_expected_return_pct": 1.0, **extra}

    def test_timesfm_chronos_agreement_boosts_score(self) -> None:
        cards = {name: {"reliability_grade": "A", "sample_size": 50} for name in ("timesfm", "chronos")}
        result = self.consensus.evaluate([self._forecast("timesfm", "UP"), self._forecast("chronos", "UP")], reliability=cards)
    ...
    def test_timesfm_chronos_disagreement_creates_warning(self) -> None:
        result = self.consensus.evaluate([self._forecast("timesfm", "UP"), self._forecast("chronos", "DOWN")])
    ...
    def test_lag_llama_high_stop_probability_degrades_score(self) -> None:
        result = self.consensus.evaluate([self._forecast("lag_llama", "UP", prob_touch_stop_before_entry=0.8)])
    ...
    def test_forecast_stack_not_called_when_setup_management_only_for_entry(self) -> None:
        config = {"setup_id": "MGMT", "symbol": "MGT", "setup_type": "position_management", "setup_role": "MANAGEMENT_ONLY"}
        self.repository.upsert_setup(SetupRecord(
            setup_id="MGMT", symbol="MGT", setup_type="position_management", enabled=True,
            mode="paper", status="IN_POSITION", entry_zone="", stop_loss=9.0, risk_amount=10.0,
            order_status="NONE", position_status="OPEN", last_event="created", config=config,
        ))
    ...
    def _comparison(self, **overrides):
        payload = {
            "symbol": "LAB", "timeframe": "15m", "horizon_bars": 1,
            "actual": [1, 2, 3, 4, 5, 6],
            "predictions": {
    ...
    def test_native_model_lab_runs_installed_provider_adapter(self) -> None:
        ...
        forecast_service = SimpleNamespace(
            ...
            stack_config=SimpleNamespace(providers={
    ...
    def test_walk_forward_no_data_leakage(self) -> None:
        result = self._comparison(validation="walk_forward", walk_forward_folds=[{"train_end": 2, "test_start": 3}])
        self.assertTrue(result["summary"]["validation"]["no_data_leakage"])
        with self.assertRaises(ValueError):
            self._comparison(validation="walk_forward", walk_forward_folds=[{"train_end": 3, "test_start": 3}])
