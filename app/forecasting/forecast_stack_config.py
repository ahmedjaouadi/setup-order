...
class ForecastStackConfig:
    ...
    def from_settings(cls, settings: dict[str, Any]) -> ForecastStackConfig:
        raw = (
            settings.get("forecast_stack")
            if isinstance(settings.get("forecast_stack"), dict)
            else {}
        )
        legacy = (
            settings.get("forecasting") if isinstance(settings.get("forecasting"), dict) else {}
        )
        ...
        if not providers_raw:
            providers_raw = {
                ...
                for name in legacy.get(
                    "default_models", ["timesfm", "naive_baseline", "atr_baseline"]
                )
            }
        providers: dict[str, ForecastProviderConfig] = {}
        for index, (name, value) in enumerate(providers_raw.items()):
            ...
            providers[name] = ForecastProviderConfig(
                ...
                priority=int(options.get("priority", index)),
                role=str(options.get("role") or "comparison"),
        ...
        return cls(
            ...
            providers=providers,
            active_models=_names(
                raw.get("active_models"), ("timesfm", "naive_baseline", "atr_baseline")
            ),
